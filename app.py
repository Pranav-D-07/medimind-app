...
app = Flask(__name__)
# Use an environment-provided secret key or generate one at startup (do not hardcode)
...
def analyze():
    try:
        ...
        # --- SAFETY CHECK: If the model is extremely uncertain, abort with a safe message ---
        try:
            if primary.get('prob', 0) < 25:
                return jsonify({
                    'diagnosis': [{
                        'condition': 'Uncertain Diagnosis',
                        'probability': f"{primary.get('prob',0):.1f}%",
                        'recommendation': "⚠️ <b>My confidence is too low to provide a reliable diagnosis. Please seek urgent medical evaluation.</b>"
                    }]
                })
        except Exception:
            # if primary missing or malformed, continue to final logic below
            pass
        ...
        # Warn about critical alternatives even when primary is not critical
        try:
            critical_alternatives = [c for c in candidates[1:] if c['severity'] == 'CRITICAL' and c['prob'] > 5]
            if critical_alternatives and primary['severity'] != 'CRITICAL':
                alt = critical_alternatives[0]
                reco += f"<br><br>⚠️ <b>Note:</b> Cannot fully rule out {alt['disease']} ({alt['prob']:.1f}%). Seek prompt medical attention."
        except Exception:
            pass
        ...
        traceback.print_exc()
        return jsonify({'recommendation': '⚠️ An unexpected error occurred. Please refresh and try again.'}), 500
