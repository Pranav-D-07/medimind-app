...
def generate_dataset(num_samples_per_disease=200):
    ...
    for disease, rules in DISEASE_RULES.items():
        for _ in range(num_samples_per_disease):
            ...
            row['gender'] = random.choice([0, 1])  # 0 female, 1 male

            # Add Mandatory Symptoms (High Probability: 85-100%)
            for sym in rules["mandatory"]:
                if random.random() < 0.92:
                    row[sym] = 1

            # Add Optional Symptoms (Medium Probability: 55-75%)
            for sym in rules["optional"]:
                if random.random() < 0.65:
                    row[sym] = 1

            # Noise: randomly flip 2-5 unrelated symptoms (5-15% noise overall)
            num_noise = random.randint(2, 5)
            available_noise = [s for s in ALL_SYMPTOMS if row[s] == 0]
            noise_symptoms = random.sample(available_noise, min(num_noise, len(available_noise)))
