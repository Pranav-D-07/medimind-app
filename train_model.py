...
def train():
    ...
    print(f"   -> Overall Accuracy: {report.get('accuracy', 0):.2%}")
    print(f"   -> Macro F1-Score: {report.get('macro avg', {}).get('f1-score', 0):.2%}")
    ...
    meta = {
        ...
        'evaluation': {
            ...
            'weak_diseases': [(d, stats['f1-score']) for d, stats in report.items() if isinstance(stats, dict) and stats.get('f1-score', 1) < 0.70]
        }
    }
